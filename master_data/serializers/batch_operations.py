...
class StringBatchUpdateRequestSerializer(serializers.Serializer):
    ...
    def validate_updates(self, value):
        ...
        if len(string_ids) != len(set(string_ids)):
            raise serializers.ValidationError(
                "Duplicate string IDs in updates")
    ...
    def validate(self, attrs):
        ...
        if request and updates:
            ...
            workspaces = strings.values_list(
                'workspace_id', flat=True).distinct()
            if len(workspaces) > 1:
                raise serializers.ValidationError(
...
class ConflictSerializer(serializers.Serializer):
    ...
    suggested_resolution = serializers.CharField(
...
class StringBatchUpdateResponseSerializer(serializers.Serializer):
    ...
    inheritance_updates = serializers.ListField(
        child=InheritanceUpdateSerializer())
...
class RollbackRequestSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        """Validate target entities based on rollback type."""
...
class RollbackResponseSerializer(serializers.Serializer):
    ...
    success = serializers.BooleanField()
    rolled_back_strings = serializers.ListField(
...
class StringUpdateBatchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'initiated_at',
                            'completed_at', 'progress_percentage', 'is_complete']
...
class MultiOperationSerializer(serializers.Serializer):
    ...
    def validate_operations(self, value):
        ...
        for i, operation in enumerate(value):
            ...
            if 'data' not in operation:
                raise serializers.ValidationError(
                    f"Operation {i} missing 'data' field")
            ...
            if not isinstance(data, (dict, list)):
                raise serializers.ValidationError(
                    f"Operation {i} 'data' must be an object or array")

            # If it's a list, validate each item
            if isinstance(data, list):
                if not data:
                    raise serializers.ValidationError(
                        f"Operation {i} 'data' array cannot be empty")
                for j, item in enumerate(data):
                    if not isinstance(item, dict):
                        raise serializers.ValidationError(
                            f"Operation {i} data item {j} must be an object")
    ...
    def execute(self, workspace_id, user):
        ...
        for i, operation in enumerate(operations):
            try:
                ...
                data_items = op_data if isinstance(
                    op_data, list) else [op_data]
                operation_results = []

                for j, data_item in enumerate(data_items):
                    if op_type == 'create_string':
                        result = self._create_string(
                            data_item, workspace_id, user)
                    elif op_type == 'update_string':
                        result = self._update_string(
                            data_item, workspace_id, user)
                    elif op_type == 'delete_string':
                        result = self._delete_string(
                            data_item, workspace_id, user)
                    elif op_type == 'create_string_detail':
                        result = self._create_string_detail(
                            data_item, workspace_id, user)
                    elif op_type == 'update_string_detail':
                        result = self._update_string_detail(
                            data_item, workspace_id, user)
                    elif op_type == 'delete_string_detail':
                        result = self._delete_string_detail(
                            data_item, workspace_id, user)
                    elif op_type == 'create_submission':
                        result = self._create_submission(
                            data_item, workspace_id, user)
                    elif op_type == 'update_submission':
                        result = self._update_submission(
                            data_item, workspace_id, user)
                    elif op_type == 'delete_submission':
                        result = self._delete_submission(
                            data_item, workspace_id, user)
                    elif op_type == 'update_string_parent':
                        result = self._update_string_parent(
                            data_item, workspace_id, user)
                    else:
                        raise serializers.ValidationError(
                            f"Unknown operation type: {op_type}")
    ...
    def _update_string(self, data, workspace_id, user):
        ...
        string = models.String.objects.get(
            id=string_id, workspace_id=workspace_id)
    ...
    def _update_string_detail(self, data, workspace_id, user):
        ...
        detail = models.StringDetail.objects.get(
            id=detail_id, workspace_id=workspace_id)
    ...
    def _update_submission(self, data, workspace_id, user):
        ...
        submission = models.Submission.objects.get(
            id=submission_id, workspace_id=workspace_id)
    ...
    def _update_string_parent(self, data, workspace_id, user):
        ...
        parent = models.String.objects.get(
            id=parent_id, workspace_id=workspace_id)
